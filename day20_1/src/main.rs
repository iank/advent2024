...
fn find_cheat_ends(start: (usize, usize)) -> Vec<(usize, usize)> {
    let mut result = vec![(start.0 + 2, start.1), (start.0, start.1 + 2)];

    if start.0 > 1 {
        result.push((start.0 - 2, start.1));
    }
    if start.1 > 1 {
        result.push((start.0, start.1 - 2));
    }
    result
}
...
fn compute_cheats(path: Vec<(usize, usize)>) -> Vec<usize> {
    ...
    for cheat_start in path {
        let cheat_ends = find_cheat_ends(cheat_start);
        for cheat_end in cheat_ends {
            if pathscores.contains_key(&cheat_end) {
                let current_score = pathscores.get(&cheat_start).unwrap();
                let new_score = pathscores.get(&cheat_end).unwrap();
                if *new_score > *current_score + 2 {
                    result.push(new_score - current_score - 2);
                }
            }
        }
    }
