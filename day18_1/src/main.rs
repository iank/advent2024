...
fn calc_new_positions(p: (usize, usize), n: usize) -> Vec<(usize, usize)> {
    ...
    for offset in offsets {
        ...
        if new_position.0 < 0 || new_position.1 < 0 || new_position.0 >= n as isize || new_position.1 >= n as isize {
            continue;
        }
...
fn bfs_search_maze(maze: &Grid<char>, start: (usize, usize), exit: (usize, usize), n: usize) -> Option<usize> {
...
fn main() -> Result<(), std::io::Error> {
    ...
    if args.len() < 4 {
        eprintln!("Usage: {} <file_path> <size> <nbytes>", args[0]);
        std::process::exit(1);
    }
...
//    println!("{:#?}", maze);
    if let Some(shortest_path_len) = bfs_search_maze(&maze, start, exit, n) {
